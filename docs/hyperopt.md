...
### Ubuntu auto-start without manual commands

If you normally use the project virtualenv with:

```bash
source .venv/bin/activate
```

then you can use a launcher script to start both the live bot and the scheduler with a single command, without manually re-entering the commands every time.
...
This starts both processes from the same project root, loads the correct virtualenv, and resolves the config path automatically. It is the simplest Ubuntu-based way to run the trading bot and the scheduled Hyperopt job without any manual terminal steps.

To make the same behavior automatic after boot, register the script as a `systemd` service:
...
This keeps the bot and the scheduler active after system startup without user interaction.
